...
void Add() {                   //void function hai so it wont return anything
    ...
    int Id = Next_ID();    //assigning value to the newID variable by calling the Next id function
    ...
    file<<setw(20)<<left<<Id<<setw(20)<<first_name<<setw(20)<<last_name<<fixed<<setprecision(2)<<setw(20)<<balance<<setw(20)<<branch<<endl;  //file mai output krwa raha hun

    file.close();    //closing the bank_management file

    cout << "\nEntry added successfully with ID: " << Id << endl;    //printing the given id on the console

    updateid(Id + 1);                 // Updating the temporary id file for the next ID
...
void search(){
  ...
  while(fin>>id>>first_name>>last_name>>balance>>branch)
    { 
      if(id == targetid){
    ...
    if(idfound){
      ...
      cout<<"\n";
      cout<<setw(20)<<left<<id<<setw(20)<<first_name<<setw(20)<<last_name<<fixed<<setprecision(2)<<setw(20)<<balance<<setw(20)<<branch<<"\n"<<endl;
    }
    else{
...
void update(){
  ...
  while(i_file>>id>>first_name>>last_name>>balance>>branch){              //reading from the bank_management file 
        
        if(id == change_id){                             //comparing the id with the user entered id
        ...
        o_file<<setw(20)<<left<<id<<setw(20)<<first_name<<setw(20)<<last_name<<fixed<<setprecision(2)<<setw(20)<<balance<<setw(20)<<branch<<endl;
  }
...
void deleteid() {
    ...
    while (file>>id>>first_name>>last_name>>balance>>branch) {
            
            if(id == target_id){                            //checking whether the id is present in the file or not
                check = true;
            }

            if (id  !=  target_id) {
                 //writing data to temp file
                tempfile<<setw(20)<<left<<id<<setw(20)<<first_name<<setw(20)<<last_name<<fixed<<setprecision(2)<<setw(20)<<balance<<setw(20)<<branch<<endl;
            }
        }
